...
| TC45 - Create Todo | Verify POST `/todos` creates a todo. | API is reachable | 1. Send POST request to `/todos` with JSON body| Status 201 and response matches payload |
...
| TC48 - Delete Todo | Verify DELETE `/todos/1` deletes a todo. | API is reachable | 1. Send DELETE request to `/todos/1` | Status 200-299 |
...
| TC52 - Create User | Verify POST `/users` creates a user. | API is reachable | 1. Send POST request to `/users` with JSON body | Status 201 and response matches payload |
